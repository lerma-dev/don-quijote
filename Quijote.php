...
// 3. Validar presencia de la API Key
if (!$apiKey) {
    echo json_encode(["error" => "❌ DEEPSEEK_API_KEY no configurada en el sistema ni en .env"]);
    exit;
}
...
/* ============================================================
    4. PROMPT DEL SISTEMA Y MENSAJES (OPTIMIZADO PARA TOKENS)
============================================================ */
$system_prompt = "Eres Don Quijote. 
    Hablas en español antiguo/caballeresco pero accesible. 
    Reglas: Máximo 2 o 3 oraciones por respuesta. 
    No hagas preguntas.
    No uses asteriscos.
    No uses emojis. 
...
if ($es_resumen) {
    $system_prompt = "Resume esta conversación en máximo 2 oraciones con estilo caballeresco breve.";
}
...
// REDUCCIÓN CLAVE: Pasar de 20 a 6 u 8 mensajes de contexto
$limite_mensajes = 6; 
...
foreach ($historial as $turno) {
    if (isset($turno["role"], $turno["content"])) {
        // Truncar contenido del historial a 300 caracteres max para no arrastrar textos gigantes
...
echo json_encode([
    ...
    "metrics" => [
        ...
        "estimated_tokens" => ceil($payload_size / 4)
    ]
]);
?>